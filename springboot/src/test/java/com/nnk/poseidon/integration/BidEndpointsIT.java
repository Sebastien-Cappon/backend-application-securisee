...
public class BidEndpointsIT {
	...
	@Order(4)
	@WithMockUser(username = "user", roles = { "USER" })
	public void get_bidUpdateForm_shouldThrowNoContent() throws Exception {
	...
	@Order(10)
	@WithMockUser(username = "user", roles = { "USER" })
	public void deleteBid_fromBidListPage_shouldThrowNoContent() throws Exception {
		mockMvc.perform(delete("/bid/delete/{id}", "9999")
				.with(csrf()))
			.andExpect(status().isNoContent());
	}
}